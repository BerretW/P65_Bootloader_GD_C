...
;
; _kbinput - wait for a key press and return with its assigned ASCII code in A.
; _kbget   - wait for a key press and return with its unprocessed scancode in A.
; _kbscan  - Scan the keyboard for 105uS, returns 0 in A if no key pressed.
;           Return ambiguous data in A if key is pressed.  Use _kbinput OR _kbget
;           to get the key information.  You can modify the code to automatically
;           jump to either routine if your application needs it.
; _kbinit  - Initialize the keyboard and associated variables and set the LEDs
;
;****************************************************************************
;
...
;               jsr   _kbinit            ; init the keyboard, LEDs, and flags
;lp0            jsr   print_cr          ; prints 0D 0A (CR LF) to the terminal
;lp1            jsr   _kbinput           ; wait for a keypress, return decoded ASCII code in A
...
; Decoding routines
;
; _kbinput is the main routine to call to get an ascii char from the keyboard
...
kbreinit:       jsr   _kbinit            ;
_kbinput:        jsr   kbtscrl           ; turn off scroll lock (ready to input)
               bne   _kbinput           ; ensure its off
_kbinput1:       jsr   _kbget             ; get a code (wait for a key to be pressed)
               jsr   kbcsrch           ; scan for 14 special case codes
kbcnvt:         beq   _kbinput1          ; 0=complete, get next scancode
...
kbcnvt3:        lda   special           ;
               ...
               beq   _kbinput1          ; ensure mask didn't leave 0
               tax                     ;
               bra   kbdone            ;
kbcnvt4:        lda   ASCIITBL,x        ; get ascii code
               beq   _kbinput1          ; if ascii code is 0, invalid scancode, get another
...
kbnull:         lda   #$00              ; set caps, get next code
               rts                     ;
;
kbExt:          jsr   _kbget             ; get next code
...
kbexrls:        jsr   _kbget             ;
               ...
               bra   kbnull            ; return with 0 in A
;
kbrlse:         jsr   _kbget             ; test for shift & ctrl
               cmp   #$12              ;
...
kbBrk1:         jsr   _kbget             ; get scancode
               dex                     ;
               bne   kbBrk1            ;
...
;
; _kbscan will scan the keyboard for incoming data for about
; 105uS and returns with A=0 if no data was received.
; It does not decode anything, the non-zero value in A if data
; is ready is ambiguous.  You must call _kbget or _kbinput to
; get the keyboard data.
;
_kbscan:         ldx   #$40              ; timer: x = (cycles - 40)/13   (105-40)/13=5
               lda   kbportddr         ;
...
_kbscan1:       lda   #clk+data              ;
               bit   kbportreg         ;
               beq   _kbscan2           ; if clk goes low, data ready
               dex                     ; reduce timer
               bne   _kbscan1           ; wait while clk is high
               jsr   kbdis             ; timed out, no data, disable receiver
               lda   #$00              ; set data not ready flag
               rts                     ; return
_kbscan2:       jsr   kbdis             ; disable the receiver so other routines get it
...
;               rts                     ; return (A<>0, A=clk bit mask value from kbdis)
               jmp   _kbinput           ; if key pressed, decode it with _kbinput
;               jmp   _kbget             ; if key pressed, decode it with _kbget
;
;

_kbscan3:      lda #clk+data
              ldy #40
:	            dey
              beq lc08c
              bit kbportreg
...
kback:          jsr   kbhighlow         ;
               ...
               bne   _kbinit            ; VERY RUDE error handler - re-init the keyboard
kbsend2:        lda   kbportreg         ;
               and   #clk              ;
               beq   kbsend2           ; wait while clk low
               bra   kbdis             ; diable kb sending
;
; _kbget waits for one scancode from the keyboard
;
...
_kbget1:         lda   #clk              ;
               bit   kbportreg         ;
               bne   _kbget1            ; wait while clk is high
               lda   kbportreg         ;
               and   #data             ; get start bit
               bne   _kbget1            ; if 1, false start bit, do again
_kbget2:         jsr   kbhighlow         ; wait for clk to return high then go low again
...
_kbget3:         dex                     ; dec bit counter
               bne   _kbget2            ; get next bit if bit count > 0
               jsr   kbhighlow         ; wait for parity bit
               beq   _kbget4            ; if parity bit 0 do nothing
               inc   parity            ; if 1, set parity to 1
_kbget4:         tya                     ; get parity count
               ply                     ;
               plx                     ;
...
kbdis:          lda   kbportreg         ; disable kb from sending more data
               ...
               rts                     ;
;
_kbinit:         lda   #$02              ; init - num lock on, all other off
               sta   special           ;
_kbinit1:        lda   #$ff              ; keybrd reset
               jsr   kbsend            ; reset keyboard
               jsr   _kbget             ;
               cmp   #$FA              ; ack?
               bne   _kbinit1           ; resend reset cmd
               jsr   _kbget             ;
               cmp   #$AA              ; reset ok
               bne   _kbinit1           ; resend reset cmd
